...
//
// Recommended CLI integration is by a --tls-details flag parsed using NewFromOptionalFile, with the following documentation.
//
//	--tls-details is a path to a containers-tls-details(5) file, affecting TLS behavior throughout the program.
//
...
type Config struct {
	...
	text   tlsdetails.TLSDetailsFile
	config *tls.Config // Parsed from .text, both match
}

// NewFromOptionalFile returns a Config matching a containers-tls-details.yaml file at the specified path.
// If path is "", it returns a valid Config with no settings (where config.TLSConfig() will return nil).
func NewFromOptionalFile(path string) (*Config, error) {
	if path == "" {
		return newFromTLSDetails(&tlsdetails.TLSDetailsFile{})
	}
	return NewFromFile(path)
}

// NewFromFile returns a Config matching a containers-tls-details.yaml file at the specified path.
func NewFromFile(path string) (*Config, error) {
	details, err := tlsdetails.ParseFile(path)
	if err != nil {
		return nil, err
	}
	res, err := newFromTLSDetails(details)
	if err != nil {
...
// newFromTLSDetails creates a Config from a tlsdetails.TLSDetailsFile.
func newFromTLSDetails(details *tlsdetails.TLSDetailsFile) (*Config, error) {
	res := Config{
		text:   tlsdetails.TLSDetailsFile{},
		config: &tls.Config{},
	}
	configChanged := false
	for _, fn := range []func(input *tlsdetails.TLSDetailsFile) (bool, error){
		res.parseMinVersion,
...
func (c *Config) parseMinVersion(input *tlsdetails.TLSDetailsFile) (bool, error) {
	if input.MinVersion == "" {
		return false, nil
	}
...
func (c *Config) parseCipherSuites(input *tlsdetails.TLSDetailsFile) (bool, error) {
...
func (c *Config) parseNamedGroups(input *tlsdetails.TLSDetailsFile) (bool, error) {
...
// TLSConfig returns a *tls.TLSConfig matching the provided settings.
// If c contains no settings, it returns nil.
// Otherwise, the returned *tls.TLSConfig is freshly allocated and the caller can modify it as needed.
...
type marshaledSerialization struct {
	Version int
	Data    tlsdetails.TLSDetailsFile
}
