...
func (w *Writer) writeLegacyMetadataLocked(layerDescriptors []manifest.Schema2Descriptor, configBytes []byte, repoTags []reference.NamedTagged) error {
	...
	for i, l := range layerDescriptors {
		...
		if i == len(layerDescriptors)-1 {
			var config map[string]*jsonv1.RawMessage
		...
		b, err := jsonv1.Marshal(layerConfig) // Note that layerConfig["id"] is not set yet at this point.
		if err != nil {
		...
		configBytes, err := jsonv1.Marshal(layerConfig)
		if err != nil {
			return fmt.Errorf("marshaling layer config: %w", err)
		}
...
func (w *Writer) Close() error {
	...
	b, err := jsonv1.Marshal(&w.manifest)
	if err != nil {
		return err
	}
	...
	b, err = jsonv1.Marshal(w.repositories)
	if err != nil {
